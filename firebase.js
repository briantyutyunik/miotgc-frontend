...
export async function userSignUp(newUser) {
	...
	try {
		...
		let userCredential = await auth.createUserWithEmailAndPassword(auth.getAuth(), newUser.email, newUser.password);
...
export async function createInvite(groupId, inviterId) {
	...
	const inviteDocRef = await addDoc(collection(firestore.getFirestore(), "invites"), inviteData);
...
export const listenGroupNames = (onGroupNameUpdate) => {
	...
	const unsubscribe = onSnapshot(query(groupsRef, where("members", "array-contains", uid)), (querySnapshot) => {
...
export async function checkIfValueExists(collection, field, value) {
	const db = firestore.getFirestore();
	const query = firestore.query(firestore.collection(db, collection), firestore.where(field, "==", value));
	const querySnapshot = await firestore.getDocs(query);
