...
const SignUpScreen = () => {
	...
	async function handleEmailChange(text) {
		...
		if (!validateEmail(text)) {
			...
			setEmailError("There's an account registered with this email address already. Sign in below if you already have an account.");
			setIsEmailValid(false);
		} else {
			setEmailError(null);
	...
	const handlePasswordChange = (text) => {
		...
		if (!validatePassword(text)) {
			setPasswordError("Password must be at least 8 characters, contain one uppercase letter, one lowercase letter, one digit, and one special character.");
		} else {
	...
	}, [firstName, lastName, email, password, confirmPassword, dob, isUsernameValid, isEmailValid, phoneNumber]);
	...
	return (
		<Background additionalStyle={styles.container}>
            ...
            <Line width={70} color={PRIMARY_COLOR} height={1} /> */}
					...
					/>
					<ImageSelect openImageSelect={openImageSelect} setOpenImageSelect={(openImageSelect) => setOpenImageSelect(openImageSelect)} setImage={(image) => setImage(image)} />
					<ScrollView contentContainerStyle={styles.scrollViewContainer} style={{ width: "100%" }}>
						<AuthInput placeholder="First Name" onChangeTextHandler={(text) => setFirstName(text)} />
						<AuthInput placeholder="Last Name" onChangeTextHandler={(text) => setLastName(text)} />
						<AuthInput placeholder="User Name" onChangeTextHandler={handleUserNameChange} error={userNameError} />
						<AuthInput placeholder="Email Address" inputType="email" secure={false} onChangeTextHandler={handleEmailChange} error={emailError} />
						<AuthInput placeholder="Password" inputType="password" secure={true} onChangeTextHandler={handlePasswordChange} error={passwordError} />
						<AuthInput placeholder="Confirm Password" inputType="password" secure={true} onChangeTextHandler={handleConfirmPasswordChange} error={confirmPasswordError} />

						<AuthInput placeholder="dob mm/dd/yyyy" secure={false} onChangeTextHandler={handleDobChange} error={dobError} />
						<AuthInput placeholder="Phone Number" keyboardType="number-pad" secure={false} onChangeTextHandler={(text) => setPhoneNumber(text)} />
					</ScrollView>
				</Card>
				{isLoading ? (
					<ActivityIndicator animating={true} useNative size="large" color={PRIMARY_COLOR} />
				) : (
					<Button
